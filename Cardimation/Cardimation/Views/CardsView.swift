...
public final class CardsView: UIView {
    ...
    /// Initialise cards animation view with all required data
    /// - Parameter cards: Cards representations that will appear on view
    /// - Parameter numberOfSlots: Number of slots that will be visible
    /// - Parameter configuration: Configuration of the cards and animation configuration
    ...
    /// Setup cards view and start the animation
    public func show() {
    ...
    private func setupNextSlot() {
        guard let card = currentCard?.element(position: numberOfSlots), let slot = cardsSlots.last else { return }
    ...
    private func createSlots(numberOfSlots: Int) -> [CardSlotView] {
        return (0...numberOfSlots).map { index in
            ...
            guard let card = currentCard?.element(position: index) else { return slot }
            slot.setup(card: card.value.create(), overlayConfiguration: card.value.overlayConfiguration)
            return slot
        }
    }
